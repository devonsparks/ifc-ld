...
IFC-LD is a data model and family of serializations designed to support the exchange of information about the built environment. It provides a standard set of pre-defined "Containers" - Packages, Objects, and Slots - in which stakeholders can store and exchange information. It is interoperable with all existing versions of IFC, while supporting integration with alternative data models through use of linked data. Practically, IFC-LD aims to replace ISO 10303-21 ("P21" files) as the leading exchange format for IFC. 
...
[source, json]
----
include::assets/json/slots.sample.json[]
----
...
Slot data is stored in its "contents". The structure of _contents_ depends on the Slot. For Object references, contents provides the @id of the targeted Object. For property values, "contents" is made up of a "@type" and  "value". Composite values (lists, sets) are made up of @type and "values" (an array), represented as a JSON-LD @list to preserve order. 
...
Slots _may_ also declare a "shortcut" property, whose value is a URI, specifying the relationship the Slot defines between its linked Objects (via _slotof_) and its _contents_. For example, a Slot may declare its linked Object is related to its Object reference contents via https://w3c-lbd-cg.github.io/bot/[Building Topology Ontology]'s "hasElement" relation: 
...
[source,json]
----
include::assets/json/object.sample.json[]
----
...
IFC-LD streams are useful for event-driven systems and one-at-a-time processing. They encourage the the exchange of "delta updates", wherein individual Slots, Objects, and Packages can be exchanged, instead of monolithic model files. 
...
Future versions of the IFC-LD specification may introduce at least one JSON serialization that is derived from, but not losslessly convertible to, the stream or package formats. These serializations would be tailored to specific use cases (e.g., visualization) where small file size is the key objective. 
...
There's a growing recognition within the industry to support https://en.wikipedia.org/wiki/Composition_over_inheritance["composition modeling"] of built asset data. Property Sets within the existing IFC specification support one flavor of this, in that multiple parties can attach different pieces of data to the same asset without prior coordination. How far can this model be taken and how can IFC-LD help?

IFC-LD supports compositional modeling at its core. Objects, which represent assets that persist across time and stakeholder revision, are primarily identified by a URI. Slots, describing qualifying aspects (e.g., properties, relations) of Objects, can be linked to any number of Objects via their _slotof_ values. As a result, mltiple stakeholders, at multiple points in time, can attach multiple pieces of data (Slot _contents_) to the same Object, without prior coordination. By labeling Slots as belonging to one or more Slot _groups_, Slots can be bundled into functional categories, making them behave equivalently to current IFC Property Sets. Together, Objects and Slots form the foundation of an industry standard https://en.wikipedia.org/wiki/Entity_component_system[entity component model]. 

NOTE: Compositional modeling in IFC-LD can be applied regardless of whether P21 integration is applied. The same Object may contain Slots mapped to existing IFC schemas and those developed independently without conflict. 

For example, consider the following set of Slot definitions:
...
All three Slots are the _slotof_ the same Object _2TqpgZlI94j9CZf2Mh_w89_ - in this case, some kind of building site. The third Slot is also the _slotof_ a second Object, _07rDKiqTT59BNsVJj21ptZ_, which represents a second site under the owner's control. The use of two Objects references in the third _slotof_ allows the same declaration of public access to be shared across Objects. Change the Slot value to true, and _all_ linked Objects will now record public access as allowed. 

Next, notice the provenance data attached to each Slot. Each Slot has been contributed by a different stakeholder at a different point in time. All can be composed, because they use global Object addresses to qualify what they're describing.

Finally, note that by specifying "group" labels, each of the Slots can be organized into functional categories akin to traditional IFC Property Sets. In this case, the categories are "Site Planning" and "Owner Requirements". Stakeholders may use these group labels to filter for Slots of interest.

These key features of Slots - the ability to link to multiple Objects and group by functional category - is what makes IFC-LD a compositional system at its core. 

==== Modules
To help take IFC in new directions, we recommend the compositional modeling support of IFC-LD be used to develop collections related of Slot definitions, called "Modules", that fulfill specific use cases, design processes, or functional categories. We imagine modules tailored to energy performance, massing design, construction administration, and more. Industry working groups for each module would take responsibility for the module's design and release cycle. Many modules would be publicly shared to increase interoperability, but some may be tailored to particular delivery processes or firms. All Modules _should_ supply a standard set of Slot schema definitions (see <<Schema Validation Strategy>>).
...
. Where possible, target SHACL schema definitions, called "shapes", against IFC-LD Slots. This allows every shape to remain small and independently testable. It also allows legacy constraints from the existing IFC schema to be developed on an as-needed basis.

. As part of an IFC-LD community effort, publish a SHACL "Slot Schema Starter Kit" with standard shapes for common use cases (matching a property value exactly, ensuring a number is within a range, etc). Many common constraint checks are variations on a theme, so providing templates allows new ideas to be tested with minimal overhead.

Because Slots _should_ declare at least one custom *@type*, Slot-based SHACL shapes can be easily written by targetting the Slot's *@type* using `sh:targetClass`. For example, here's a SHACL shape constraining the size of IFC4 IfcGUID Slots (defined as the first Slot of IfcRoot):
...
CAUTION: Slot _contents_ structure has been under revision and may need to be brought into alignment with SHACL sh:path targets. 
...
. All attributes are stored at their respective offset. Object references are encoded as integers.
...
. There are no schema annotations, including property names or type hints (i.e., is this element a literal or an object reference)
...
. We'd want to be able to add metadata to any object, especially to qualify the values we're encoded (e.g. against a particular schema or asset version).
...
. does not model EXPRESS schema concepts, but instead provides its own simple Package-Object-Slot metamodel
...
. encourages the design of new types and definitions as needed through Slots
...
There's been https://w3c-lbd-cg.github.io/lbd/[exciting work] on purpose-built industry linked data models over the last few years. Each of these model can be seen as a "module" describing one aspect of industry processes or structures. Being based on linked data, these modules are composable, but resolution of the resulting data is low-level: everything is a collection of (subject, predicate, object) triples. We believe this resolution is too granular for most industry use cases, especially those without linked data support. IFC-LD provides a more object-oriented definition of what a module is and allows any of those purpose-built data models to be plugged in as needed (e.g., via a Slot's _shortcut_). By analogy with electricity, IFC-LD defines the shape of a standard data socket, so anything fitting that socket can plug in. IFC-LD is effectively an higher-level abstraction and composition layer on top of these other models. 
