...
NOTE: As of fall 2022, this specification is under active community review and development. Prose and data samples may have inconsistencies. Please report all errors or concerns through Github as listed in the <<Community>> section. 

== Rationale

IFC-LD is a data model and family of serializations designed to support the exchange of information about the built environment. It provides a standard set of pre-defined "Containers" - Packages, Objects, and Slots - in which stakeholders can store and exchange information. It is interoperable with all existing versions of IFC, while supporting integration with alternative data models through use of linked data. Practically, IFC-LD aims to replace ISO 10303-21 ("P21" files) as the defacto exchange format for IFC. 

IFC-LD attempts to solve several problems with IFC-based data exchanges through use of convention:

1. To avoid use of custom parsers for ISO 10303-21 ("P21") files, IFC-LD provides a standard set of JSON-based serializations.
...
There's been https://w3c-lbd-cg.github.io/lbd/[exciting work] on purpose-built industry linked data models over the last few years. Each of these model can be seen as a "module" describing one aspect of industry processes or structures. Being based on linked data, these modules are composable, but resolution of the resulting data is low-level: a collection of (subject, predicate, object) triples. We believe this resolution is too granular for most industry use cases, especially those without linked data support. IFC-LD provides a more object-oriented definition of what a module is and allows any of those purpose-built data models to be plugged in as needed (e.g., via a Slot's _shortcut_). By analogy with electricity, IFC-LD defines the shape of a standard data socket, so anything fitting that socket can plug in. 
...
The <<Proof-of-Concept Transformation Pipeline>> manipulates this standard context in various ways to convert between intermediate encodings. 
...
link::assets/json/ifc-ld.schema.json[IFC-LD Package Format] (JSON Schema Draft 7)
